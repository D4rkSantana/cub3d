...
/*   Updated: 2023/04/09 23:57:16 by esilva-s         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	calc_dist(t_data *data, t_ray *ray)
{
	...
	{
		ray->distance = ray->vert->distance * cos(normalize_angle(data->player->angle - ray->ray_angle));
		ray->vertical_wall = 1;
	}
	else
	{
		ray->distance = ray->hrz->distance * cos(normalize_angle(data->player->angle - ray->ray_angle));
		ray->vertical_wall = 0;
	}
...
void	raycasting(t_data *data)
{
	...
	while (column_id < NUM_RAYS)
	{
		...
		// calcule_offset(data, &data->rays[column_id])
