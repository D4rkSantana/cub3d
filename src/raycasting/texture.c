...
/*   Updated: 2023/04/09 20:24:34 by esilva-s         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	calcule_off_y(int y, double wall_height, t_ray *ray)
{
	...
	if (wall_height <= 0)
		return(0);
	//distance_top = y + (ray->render->wall_strip_height / 2) - (WIN_HEIGHT / 2);
	/*
	distanceFromTop = y + (wallStripHeight / 2) - (WINDOW_HEIGHT / 2);
    textureOffsetY = distanceFromTop * ((float)TEXTURE_HEIGHT / wallStripHeight);
	*/
	//offset_y = distance_top * ((float) TILE_SIZE / ray->render->wall_strip_height);
	offset_y = (y * TILE_SIZE) / floor(wall_height);
	return (offset_y);
}
